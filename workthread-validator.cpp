/**
 * SED13 Interactive Validator
 */

// workthread.cpp
// workthread type: validator
...
void WorkThread::run() {
    ...
    while (sequence->next()) {
        ...
        // init VideoCapture
        String filename = dataset->baseLocation + "i-LIDS/Gatwick/Dev08/" + sequence->getName() + ".mpeg";
        ...
        annotation = new Interval(*sequence, "annotation_tracks");   // in fact, there are just plain KeyValues
        destruct(annotation->select);
        annotation->select = new Select(*annotation, String("SELECT a.seqname, a.t1, a.t2, event, a.track, xgtf_id, frames, positions, sizes\n")
                           + " FROM " + vtapi->commons->getDataset() + ".annotation_tracks AS a JOIN " + vtapi->commons->getDataset() + ".tracks AS t\n"
                           + "      ON (a.seqname = t.seqname and a.track = t.track)\n"
                           + " WHERE a.seqname = '"+ sequence->getName() +"'\n"
                           // + "       AND event='PersonRuns'\n"
                           + " ORDER BY event, xgtf_id;");
        ...
        while (true) {
            ...
            if (nextClicked) {
                ...
                annotation->update->where = String("seqname = '")+ sequence->getName() +"' AND xgtf_id = "+ itoa(annotation->getInt("xgtf_id")) +" AND track = " + itoa(track);
                // tohle se vodpali pri dalsim zavolani next, takze pohoda :)

     /*
                    annTrack->set
                    add(annotation->getSequenceName(), annotation->getStartTime(), annotation->getEndTime());
     ...
     */
            ...
            if (timerTick) {
                ...
                if (!vc.empty()) {
                    ...
                    cv::putText(vc, sequence->getName() + " " + itoa(frame), cv::Point(5,16), 0, 0.5, Scalar::all(0), 3);
                    cv::putText(vc, sequence->getName() + " " + itoa(frame), cv::Point(5,16), 0, 0.5, Scalar(0, 255, 255), 1);
